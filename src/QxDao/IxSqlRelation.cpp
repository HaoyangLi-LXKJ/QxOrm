...
struct Q_DECL_HIDDEN IxSqlRelation::IxSqlRelationImpl
{
  ...
  const QxSoftDelete& getSoftDelete(QxSqlRelationParams& params) const
  {
    ...
    if ((! pSession) || (! m_pClass))
    {
      return m_oSoftDelete;
    }
...
bool IxSqlRelation::canInit() const
{
...

  {
    ...
    return (! m_pImpl->m_bInitInEvent);
  }
}

void IxSqlRelation::setClass(IxClass* pClass, IxClass* pClassOwner)
{
  m_pImpl->m_pClass = pClass;
  m_pImpl->m_pClassOwner = pClassOwner;
}
...
void IxSqlRelation::init()
{
  ...
  QString sCheckMsg = "Check relationship '" + this->getKey() + "' from '" + (m_pImpl->m_pClassOwner ? m_pImpl->m_pClassOwner->getKey() : QString()) + "' to '" +
                      (m_pImpl->m_pClass ? m_pImpl->m_pClass->getKey() : QString()) + "'";

  if (! m_pImpl->m_pClass)
  {
    QString sAssertMsg = sCheckMsg + " : m_pImpl->m_pClass is equal to NULL";
    qAssertMsg(false, "[QxOrm] qx::IxSqlRelation::init()", qPrintable(sAssertMsg));
  }

  if (! m_pImpl->m_pClassOwner)
  {
    QString sAssertMsg = sCheckMsg + " : m_pImpl->m_pClassOwner is equal to NULL";
    qAssertMsg(false, "[QxOrm] qx::IxSqlRelation::init()", qPrintable(sAssertMsg));
  }

  if (! m_pImpl->m_pDataMember)
  {
    QString sAssertMsg = sCheckMsg + " : m_pImpl->m_pDataMember is equal to NULL";
    qAssertMsg(false, "[QxOrm] qx::IxSqlRelation::init()", qPrintable(sAssertMsg));
  }

  if (! m_pImpl->m_pDataMemberX)
  {
    QString sAssertMsg = sCheckMsg + " : m_pImpl->m_pDataMemberX is equal to NULL";
    qAssertMsg(false, "[QxOrm] qx::IxSqlRelation::init()", qPrintable(sAssertMsg));
  }

  if (! m_pImpl->m_pDataMemberId)
  {
    QString sAssertMsg = sCheckMsg + " : m_pImpl->m_pDataMemberId is equal to NULL";
    qAssertMsg(false, "[QxOrm] qx::IxSqlRelation::init()", qPrintable(sAssertMsg));
  }
  ...
  {
    QString sTraceMsg = "[QxOrm] Init relationship '" + this->getKey() + "' from '" + (m_pImpl->m_pClassOwner ? m_pImpl->m_pClassOwner->getKey() : QString()) + "' to '" +
                        (m_pImpl->m_pClass ? m_pImpl->m_pClass->getKey() : QString()) + "'";
    qDebug() << sTraceMsg;
  }
  ...
  {
    p = m_pImpl->isValid_DataMember(l);
    ...
    {
      QString sDebugMsg = "[QxOrm] Relationship '" + this->getKey() + "' from '" + (m_pImpl->m_pClassOwner ? m_pImpl->m_pClassOwner->getKey() : QString()) + "' to '" +
                          (m_pImpl->m_pClass ? m_pImpl->m_pClass->getKey() : QString()) + "' : data member '" + p->getKey() + "' already exists in the collection";
      qDebug() << sDebugMsg;
    }
  ...
  for (long l = 0; l < lCount; ++l)
  {
    p = m_pImpl->isValid_SqlRelation(l);
    ...
    {
      QString sDebugMsg = "[QxOrm] Relationship '" + this->getKey() + "' from '" + (m_pImpl->m_pClassOwner ? m_pImpl->m_pClassOwner->getKey() : QString()) + "' to '" +
                          (m_pImpl->m_pClass ? m_pImpl->m_pClass->getKey() : QString()) + "' : relation '" + p->getKey() + "' already exists in the collection";
      qDebug() << sDebugMsg;
    }
...
bool IxSqlRelation::verifyOffset(QxSqlRelationParams& params, bool bId) const
{
  ...
  if ((! qx::QxSqlDatabase::getSingleton()->getVerifyOffsetRelation()) || (params.isDistinct()))
  {
    return true;
  }
...
QVariant IxSqlRelation::getIdFromQuery_ManyToMany(bool bEager, QxSqlRelationParams& params, int iOffset, int iNameIndex) const
{
  ...
  else if (params.isDistinct())
  {
    ...
    while ((p = this->nextData(lIndex)))
    {
      ...
      {
        sId += params.query().value(iOffset + lCurrIndex).toString() + "|";
        lCurrIndex++;
      }
    }
...
void IxSqlRelation::updateOffset_ManyToMany(bool bEager, QxSqlRelationParams& params) const
{
  ...
  {
    lOffsetNew = (lOffsetNew - params.getColumnsOffset());
    params.setOffset(lOffsetNew);
  }
  else if (bEager && (params.getColumnsCount() > 0))
  {
    long l1 = 0;
    IxDataMember* p = NULL;
    long lColumnsOffset = 0;

    while ((p = this->nextData(l1)))
    {
      if (! params.checkColumns(p->getKey()))
      {
        lOffsetNew = (lOffsetNew - 1);
        lColumnsOffset++;
      }
    }

    params.setOffset(lOffsetNew);
    params.setColumnsOffset(lColumnsOffset);
  }
...
void IxSqlRelation::eagerSelect_ManyToMany(QxSqlRelationParams& params) const
{
  ...
  {
    sql += (pId->getSqlTablePointNameAsAlias(tableAlias, ", ", "", false, "", (& params.builder())) + ", ");
  }
  ...
  while ((p = this->nextData(l1)))
  {
    if (params.checkColumns(p->getKey()))
    {
      sql += (p->getSqlTablePointNameAsAlias(tableAlias, ", ", "", false, "", (& params.builder())) + ", ");
    }
  }
  ...
  {
    sql += (oSoftDelete.buildSqlTablePointName(tableAlias) + ", ");
  }
}
...
void IxSqlRelation::eagerSelect_ManyToOne(QxSqlRelationParams& params) const
{
  ...
  if (params.indexOwner() > 0)
  {
    sSuffixAlias = "_" + QString::number(params.indexOwner());
  }

  if (pData && (! params.isDistinct()))
  {
    sql += (pData->getSqlTablePointNameAsAlias(tableRef, ", ", sSuffixAlias, false, "", (& params.builder())) + ", ");
  }
...
void IxSqlRelation::eagerSelect_OneToMany(QxSqlRelationParams& params) const
{
  ...
  QString& sql = params.sql();
  IxSqlRelation* pRelation = NULL;
  IxDataMember* pForeign = this->getDataByKey(this->m_pImpl->m_sForeignKey);
  qAssert(pForeign);
  IxDataMember* p = NULL;
  IxDataMember* pId = this->getDataId();
  qAssert(pId);
  ...
  if (pForeign && (! params.isDistinct()))
  {
    sql += (pForeign->getSqlTablePointNameAsAlias(tableAlias, ", ", "", false, "", (& params.builder())) + ", ");
  }
...
{
  QString& sql = params.sql();
  IxDataMember* pId = this->getDataIdOwner();
  qAssert(pId);
  IxDataMember* pForeign = this->getDataByKey(this->m_pImpl->m_sForeignKey);
  qAssert(pForeign);
  QString table = this->table();
...
void IxSqlRelation::eagerJoin_OneToOne(QxSqlRelationParams& params) const
{
  ...
  IxDataMember* pId = this->getDataId();
  qAssert(pId);
  IxDataMember* pIdRef = this->getDataIdOwner();
  qAssert(pIdRef);
  QString table = this->table();
  QString tableAlias = this->tableAlias(params);
  QString tableRef = this->tableAliasOwner(params);
...
void IxSqlRelation::eagerWhereSoftDelete_ManyToMany(QxSqlRelationParams& params) const
{
  ...
  if (oSoftDelete.getSqlFetchInJoin())
  {
    return;
  }
...
void IxSqlRelation::lazyInsert_ManyToOne(QxSqlRelationParams& params) const
{
  ...
  if (pData)
  {
    sql += pData->getSqlName(", ", "", false, (& params.builder())) + ", ";
  }
}
...
void IxSqlRelation::lazyUpdate_ManyToOne(QxSqlRelationParams& params) const
{
  ...
  if (pData)
  {
    sql += pData->getSqlNameEqualToPlaceHolder("", ", ", false, (& params.builder())) + ", ";
  }
}
...
void IxSqlRelation::createTable_ManyToOne(QxSqlRelationParams& params) const
{
  ...
  if (pData)
  {
    sql += pData->getSqlNameAndTypeAndParams(", ") + ", ";
...
QString IxSqlRelation::createExtraTable_ManyToMany() const
{
  ...
  bool bOldPKOwner = pIdOwner->getIsPrimaryKey();
  pIdOwner->setIsPrimaryKey(false);
  bool bOldPKData = ((pIdOwner == pIdData) ? bOldPKOwner : pIdData->getIsPrimaryKey());
  pIdData->setIsPrimaryKey(false);
  bool bOldAIOwner = pIdOwner->getAutoIncrement();
  pIdOwner->setAutoIncrement(false);
  bool bOldAIData = ((pIdOwner == pIdData) ? bOldAIOwner : pIdData->getAutoIncrement());
  pIdData->setAutoIncrement(false);

  QString sql = "CREATE TABLE IF NOT EXISTS " + this->m_pImpl->m_sExtraTable + " (";
  sql += pIdOwner->getSqlNameAndTypeAndParams(", ", this->m_pImpl->m_sForeignKeyOwner) + ", ";
  qAssert(! pIdOwner->getSqlType().isEmpty());
  sql += pIdData->getSqlNameAndTypeAndParams(", ", this->m_pImpl->m_sForeignKeyDataType) + ", ";
  qAssert(! pIdData->getSqlType().isEmpty());
  sql = sql.left(sql.count() - 2); // Remove last ", "
  sql += ")";

  pIdOwner->setIsPrimaryKey(bOldPKOwner);
  pIdData->setIsPrimaryKey(bOldPKData);
  pIdOwner->setAutoIncrement(bOldAIOwner);
  pIdData->setAutoIncrement(bOldAIData);
